...
void CDispOptimizeICGN_CPU::run(cv::Mat &_l_image, cv::Mat &_r_image, cv::Mat &_src_disp, int subset, int sideW, int maxIter,
    ...
    cv::Mat localSubHom = cv::Mat(subset, subset, CV_32SC3);
    localSubHom.setTo(0);
...
int CDispOptimizeICGN_CPU::calLocalSubHom_cv(int subset, cv::Mat &_localSubHom, cv::Mat &_localSub){
    cv::Mat tmp_channel_0 = cv::Mat(_localSubHom.rows, _localSubHom.cols, CV_32SC1 );
    int halfSubset = subset / 2;
    for (int iRow = 0; iRow < subset; ++iRow) {
        int value = -halfSubset + iRow;
        for (int iCol = 0; iCol < subset; ++iCol) {
            tmp_channel_0.at<int>(iRow, iCol) = value;
        }
    }
    cv::Mat tmp_channel_1 = cv::Mat(_localSubHom.rows, _localSubHom.cols, CV_32SC1 );
    tmp_channel_1 = tmp_channel_0.t();

    cv::Mat tmp_channel_2 = cv::Mat(_localSubHom.rows, _localSubHom.cols, CV_32SC1 );
    tmp_channel_2.setTo(1);
    cv::imwrite("tmp_channel_0.jpg", tmp_channel_0);
    cv::imwrite("tmp_channel_1.jpg", tmp_channel_1);
    cv::imwrite("tmp_channel_2.jpg", tmp_channel_2);
