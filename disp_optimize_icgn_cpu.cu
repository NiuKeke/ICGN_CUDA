                                    ...
                                    cv::Mat &_result)
{
    ...
    ICGNalgo(_l_image.data, _r_image.data, (float *)_x_gradient_image.data, (float *)_y_gradient_image.data,
             localSubHom, localSub, _src_disp.data, (float *)_result.data,
             _l_image.rows, _l_image.cols, subset, sideW, maxIter);
    t2 = clock();
    printf("视差优化 time: %fs\n", (double(t2 - t1)/ 1000.0f));
    delete[] localSubHom;
    delete[] localSub;
}
...
int CDispOptimizeICGN_CPU::ICGNalgo(unsigned char *srcL, unsigned char *srcR, float *gradxImg, float *gradyImg,
                                    int *localSubHom, float *localSub, uchar *disp, float *dispFloat,
...
//                if (i == 16961)
            ...
            dispFloat[i] = float(disp[i]) + dispBias[1];
            //printf("dispBias[1]: %lf,disp[%d]: %d,dispFloat[%d]: %lf\n",dispBias[1],i, disp[i],i,dispFloat[i]);
            delete[] ImRef;
            delete[] ImDef;
            delete[] gradxImgRef;
            delete[] gradyImgRef;
                                     ...
                                     int subset, int halfSubset, int sideW, int length, bool bDebug)
{
    ...
    for (int j = -halfSubset; j <= halfSubset; j++)
    {
        for (int k = -halfSubset; k <= halfSubset; k++)
        {
            ...
            //     "j:" << j << "         " << "k:" << k << "        " <<
            //     Jacobian[n] << "          " << 
            //     Jacobian[n + subset * subset] << "           " << 
            //     Jacobian[n + 2 * subset * subset] << "           " << 
            //     Jacobian[n + 3 * subset * subset]  <<"           " << 
            //     Jacobian[n + 4* subset * subset] << "           " << 
            //     Jacobian[n + 5 * subset * subset] <<std::endl;
            // }
    ...
    for (int j = 0; j < subset * subset; j++)
    {
        ...
        /*if (bDebug && j == 14){
            std::cout << "j:         " << j << "          " << deltafVec[j] << "  fSubset[j]:" << fSubset[j] << "  meanfSubset: " << meanfSubset << std::endl;
        }*/
    ...
    while (thre > 1e-3 && Iter < maxIter || Iter == 0)
    {
        ...
        if (minPcoordInt1 > 2 && minPcoordInt2 > 2 && maxPcoordInt1 < sizeX - 2 && maxPcoordInt2 < sizeY - 2)
        {
            ...
            for (int j = 0; j < subset * subset; j++)
            {
                ...
                /*if (bDebug && j == 14 && Iter == 0)
                {
                    printf("j: %d, target_delta_mean_value: %lf,meandefIntp: %lf, image_value_int: %lf\n",
                           j, deltagVec[j], meandefIntp, defIntp2[j]);
                }*/
            }
            ...
            /*if (bDebug&&  Iter == 10){
                std::cout << warP[0][0] << "        " << warP[0][1] << "        " << warP[0][2] << std::endl;
                std::cout << warP[1][0] << "        " << warP[1][1] << "        " << warP[1][2] << std::endl;
                std::cout << warP[2][0] << "        " << warP[2][1] << "        " << warP[2][2] << std::endl;
